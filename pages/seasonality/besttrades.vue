<template>
  <div>
    <Navbar />
    <Subbar title="Seasonality > Best Trades" :submenu="true" />
                ...
                <div class="card-body bg-dark">
                  <div>
                    <div class="input-group">
                      ...
                      <select v-model="query.month" class="form-control" style="background-color: rgba(204, 209, 224, 0.3); color:royalblue;">
                  ...
                  <table id="datatable" class="table table-striped table-bordered" style="word-break: break-all;">
                    <thead class="thead-light sortable">
                      <tr>
                        ...
                        <th @click="sort('Year')">
                          <i :class="iconClass('Year')" /> Year
                        </th>
                        <th @click="sort('Start_Date')">
                          <i :class="iconClass('Start_Date')" /> Start Date
                        </th>
                        <th @click="sort('Trade_Length')">
                          <i :class="iconClass('Trade_Length')" /> Trade Length
                        </th>
                    ...
                    <tbody>
                      <tr v-for="(b, i) in bests" :key="i">
                        <td>{{ b.Sym }}</td>
                        <td>{{ b.Year }}</td>
                        <td>{{ b.Start_Date }}</td>
                        <td>{{ b.Trade_Length }}</td>
                        <td>{{ b.Confidence }}</td>
                        <td>{{ b.Move }}</td>
                        <td>{{ b.MaxDrawdown }}</td>
                        <td>{{ b.MaxProfit }}</td>
...
export default {
  ...
  methods: {
    ...
    getMonthlyPredication (symbol) {
      ...
      this.$xhr.api.post('/api/seasonality', { action: 'getMonthlyPredication', symbol: this.query.code, month: this.query.month }).then((response) => {
    ...
    getBestTrades () {
      this.$xhr.api.post('/api/seasonality', { action: 'bestTrades', symbol: this.query.code, month: this.query.month, ...this.formData }).then((response) => {
        this.bests = response.data.bests
        this.show.results = true
      })
    },
    getResults () {
