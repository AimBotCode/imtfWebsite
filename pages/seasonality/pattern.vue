<template>
  <div>
    <Navbar />
    <Subbar title="Seasonality > Pattern" :submenu="true" />
            ...
            <div class="col-lg-9">
              ...
              <div v-else class="card">
                ...
                <div class="card-header bg-light">
                  <div class="row align-items-center">
                    <div class="col">
                      <FormsPattern v-if="show.filters" @change="formChanged" />
                    </div>
                  </div>
                </div>
                        ...
                        <table id="datatable" class="table table-striped table-bordered" style="word-break: break-all;">
                          ...
                          <tbody>
                            <tr v-for="(r, i) in rows" :key="i">
                              <td><a href="javascript:;" class="text-blue" @click="showSymbol(r.symname)">{{ r.symname }}</a></td>
                              <td>{{ r.Direction }}</td>
                              <td>{{ r.Confidence }}</td>
                              <td>{{ r.Move }}</td>
                              <td>{{ r.MaxDrawdown }}</td>
  ...
  methods: {
    getData () {
      this.$xhr.api.post('/api/seasonality', this.formData).then((response) => {
        this.$store.commit('app/setPatternForm', JSON.parse(JSON.stringify(this.formData)))
